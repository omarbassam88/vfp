...
Instrument::Instrument(QObject *parent, InstSocket &socket)
    : QObject(parent), m_socket(socket), m_lastCommandSent("") {
...
bool Instrument::isConnected() const { return m_socket.IsOpen(); }

QString Instrument::GetHostName() const { return m_socket.GetHostName(); }

quint16 Instrument::GetPort() const { return m_socket.GetPort(); }

void Instrument::SetLongWaitMs(int value) { m_socket.SetLongWaitMs(value); }

void Instrument::SetShortWaitMs(int value) { m_socket.SetShortWaitMs(value); }

void Instrument::WireConnections() {
  connect(&m_socket, &InstSocket::NotifyConnected, this,
          &Instrument::onConnected);
  connect(&m_socket, &InstSocket::NotifyDisconnected, this,
...
void Instrument::onPortChanged(quint16 port) { m_socket.SetPort(port); }
