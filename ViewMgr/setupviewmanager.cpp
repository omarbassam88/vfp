...
SetupViewManager::~SetupViewManager() { Utils::DestructorMsg(this); }

void SetupViewManager::WireSettings(Settings &config) {
  connect(&config, &Settings::NotifyStatusMessage, &m_setupTab,
...
void SetupViewManager::WireMessages() {
  ...
  connect(&m_instrument, &Instrument::NotifyStatusUpdated, &m_setupTab,
          &SetupTab::onStatusUpdated);
  connect(this, &SetupViewManager::NotifyStatusUpdated, &m_setupTab,
          &SetupTab::onStatusUpdated);
}
...
void SetupViewManager::WireButtons() {
  ...
  connect(&m_setupTab, &SetupTab::NotifyDiconnectClicked, &m_instrument,
